...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(c -> c.getWantToBuy().stream()).map(Item::getName).filter(item -> !itemListOnSale.contains(item)).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> {
            return customer.getBudget() >= customer.getWantToBuy().stream().map(Item::getName).mapToInt(item -> {
                return onSale.stream().filter(onSaleItem -> onSaleItem.getName().equals(item)).mapToInt(Item::getPrice).min().orElse(0);
            }).sum();
        };
